...
final class EvaluationAttemptQuotaRaceTest extends TestCase
{
    ...
    /**
     * Le mode entraînement (évaluation TERMINÉE) ignore `max_attempts` — c'est
     * un choix explicite du domaine, antérieur à ces PR : une fois l'évaluation
     * close et les notes officielles enregistrées, l'étudiant s'entraîne
     * librement. Ce test l'épingle pour que le comportement reste INTENTIONNEL
     * et visible, plutôt que découvert un jour comme une fuite de quota.
     *
     * Chaque cycle d'entraînement passe par un `/start` explicite : depuis que
     * `/submit` ne rouvre plus de tentative tout seul, la croissance des lignes
     * d'entraînement est bornée par une action délibérée de l'étudiant — la
     * dette signalée en première rédaction n'a plus lieu d'être.
     *
     * Garde-fou vérifié : ces soumissions portent le marqueur `[PRACTICE]`, que
     * le grading ne réécrit pas et que la synchronisation KLASSCI exclut
     * explicitement — aucune note d'entraînement ne remonte donc à KLASSCI.
     */
    public function test_le_mode_entrainement_ignore_le_quota_et_reste_hors_klassci(): void
    {
        $this->fakeOpenWindow();
        ...
        $this->assertCount(3, $submissions, 'Le mode entraînement autorise des tentatives illimitées.');
        foreach ($submissions as $submission) {
            $this->assertStringStartsWith(
                '[PRACTICE]',
                (string) $submission->feedback,
                'Une soumission d entraînement doit rester marquée, sinon elle partirait vers KLASSCI.',
            );
        }
    }
