...
final class KnowledgeCheckLegacyTenantAttemptTest extends TestCase
{
    ...
    /**
     * Cohérence quota ↔ historique.
     *
     * Si le quota comptait la tentative héritée sans que l'historique la
     * montre, l'étudiant serait bloqué en « quota atteint » devant une liste
     * VIDE, sans aucun moyen de comprendre. Les deux doivent voir le même jeu
     * de lignes.
     */
    public function test_lhistorique_montre_la_tentative_qui_bloque_le_quota(): void
    {
        $this->quiz->forceFill(['max_attempts' => 1])->save();
        $this->seedLegacyAttempt(attemptNumber: 1);

        $token = $this->student->createToken('legacy-history-581')->plainTextToken;
        ...
        $history = $this->withHeaders($headers)
            ->getJson("/api/knowledge-checks/{$this->quiz->id}/my-attempts");

        $history->assertStatus(200);
        $this->assertCount(
            1,
            $history->json('data.attempts'),
            'La tentative qui consomme le quota doit apparaître dans l historique.',
        );
        $this->assertSame(40, $history->json('data.best_score'));
    }
